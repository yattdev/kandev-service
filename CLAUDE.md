...
Kandev v0.92's exact exported task/session pair is verified and its workspace is
derived from the matching task; an exported workspace ID must also agree. When
no launch IDs exist, the guard requires exactly one active executor launch
matching the exact materialized task root, including the brief window where its
session row still says `WAITING_FOR_INPUT`. Other partial or mismatched IDs fail closed.
