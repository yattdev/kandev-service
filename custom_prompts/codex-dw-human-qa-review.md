...
If the receipt/artifact/runtime is absent, stale, incompatible, disposed, or insufficient for the scenario, request a refresh through the escalation chain (`subtask → parent task → workspace Coordinator`). State the full task UUID, repository/project, current receipt when any, engine/version, required scenario and desired task-local destination. Do not inspect/export an unrelated or main container yourself, and do not replace the missing fixture with ad-hoc mock data.
