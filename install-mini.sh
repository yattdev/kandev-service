...
# ── Restic backup cron (daily 15:00) ─────────────────────────────────────────
...
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
0 15 * * * root USER_NAME=$USER_NAME USER_HOME=$USER_HOME RESTIC=$RESTIC RESTIC_REPO=$RESTIC_REPO RESTIC_PASSWORD_FILE=$RESTIC_PASSWORD_FILE $BACKUP_SCRIPT_DST
EOF
chmod 644 "$BACKUP_CRON_FILE"
chown root:root "$BACKUP_CRON_FILE"
echo "Installed backup cron: $BACKUP_CRON_FILE (daily 15:00)"
...
echo "   backup cron    : daily 15:00 → $USER_HOME/logs/kandev-backup.log"
echo ""
echo "Next steps:"
